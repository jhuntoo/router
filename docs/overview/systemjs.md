...
Below is an example of a SystemJS config which maps the `@ngrx/router` to its npm package folder along with its external dependencies including `path-to-regexp`, `isarray`, `query-string` and `strict-uri-encode`. If you have an existing SystemJS config, just add the `@ngrx/router` sections to your configuration.
